...
try:
    ...
    st.warning("Background image 'icon/bgbutterfly.jpg' not found. Please ensure it's in the correct path.")
...
CSV_FILE_LARVAL_DISEASE = os.path.join(DATA_DIR, "larval_disease_quality_records.csv")
...
def display_butterfly_data():
    ...
    if not butterfly_species_detected.empty:
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        df_plot = butterfly_species_detected.melt(id_vars='Species', value_vars=['Correct Detected', 'Incorrect Detected'],
                                                  var_name='Detection Type', value_name='Count')
        sns.barplot(x="Species", y="Count", hue="Detection Type", data=df_plot, palette="Paired", ax=ax)
        plt.xticks(rotation=75, ha="right")
